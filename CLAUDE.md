...
- **Header:** static and session-free (see the comment in `src/components/header.tsx`). The "Admin console" link is shown by `src/components/header-actions.tsx`, which asks `GET /api/me` (`{admin: boolean}`, no-store) from the browser after load, so public pages stay prerendered. `/api/me` is a convenience for the link only; `/admin` still enforces permission itself via `requireAdmin()`.
