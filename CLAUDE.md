...
- **Sign-in page** (`/sign-in`): the Airy hero band with a centered card and a Google button. The band's drawing is one big globe placed two ways (`art` prop on `HeroBand`, `src/components/hero-band.tsx`): the default `corner` for the landing and members heroes -- centered on the band's bottom-right corner so only its top-left quarter shows (small on phones) -- and `large` for the sign-in and request-access pages via `AuthShell`, parked so only its left ~40% is on screen (`left-full` plus a -40% shift; smaller on phones). Lines are 1px non-scaling strokes so they stay crisp at any size. The Google button always opens the account chooser (`prompt: "select_account"` in `src/auth.config.ts`). It also renders Auth.js's `?error=AccessDenied` message. If the visitor is already signed in as an approved member it redirects to `/members` (the same `isApprovedMember` test `/members` uses, so the two can never bounce back and forth). A signed-in visitor who is *not* on the member list (an `ADMIN_EMAILS` admin who hasn't added themselves, or someone removed since signing in) stays on the page with an "Open the admin console" link (admins) and a sign-out button.
...
- **The landing page** (`/`) uses the same Airy hero band as `/members` (`src/components/hero-band.tsx`, `landing-hero.tsx`). The "Join" button is intentionally not rendered for now (`links.joinSurvey` is kept in content). The contact and social buttons are icon "flip buttons" (`src/components/flip-button.tsx`, CSS `.flip` in `globals.css`): the icon flips to a text label on hover or keyboard focus, and cross-fades instead for `prefers-reduced-motion`. Each link's accessible name is its `aria-label`; the icon glyphs live in `src/components/icons.tsx`. Board members show initials, not photos, until each person consents to a photo.
