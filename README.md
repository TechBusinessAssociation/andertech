...
- **Resources** are links (Drive, Looker Studio, a Google Form, ...). Each one belongs to a category. The table shows each label as a clickable link; click the eye icon to see the full URL. Use **Edit** on a row to change it. Tick **Pin to "Start here"** to show a resource as one of the big cards near the top of the members home page (three or fewer looks best; at most six show). The little icon and type label (Sheet, Form, Dashboard, Video, PDF, Folder, ...) come from the link itself, so there's nothing to fill in.
- **The members home page** (`/members`) is built from what you enter on `/admin`: a welcome band with the next event's countdown, the next three events, the pinned "Start here" cards, and every resource grouped by category with a search box and category filters. The greeting uses the first name on the member's Google account.
